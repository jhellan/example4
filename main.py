...
APP_BASE_URI = 'https://example4.USERNAME.repl.co'
...
def get_group_members(user_session, groupid):
    headers={"Authorization": f"Bearer {user_session.access_token}"}
    response = requests.get(GROUPS_BASE_URI + '/' + 'groups/groups/' + groupid + '/members', headers=headers)
    response.raise_for_status()
    return response.json()
...
def login1():
    ...
    for group in mygroups:
        members = get_group_members(user_session, group['id'])
        if len(members) > 0:
            ret.append({group['displayName']: members})
    return jsonify(ret)
